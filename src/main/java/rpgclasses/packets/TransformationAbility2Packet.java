...
public class TransformationAbility2Packet extends Packet {
    public final int slot;
    public final int x;
    public final int y;

    public TransformationAbility2Packet(byte[] data) {
        super(data);
    ...
    public TransformationAbility2Packet(int slot, int x, int y) {
        this.slot = slot;
        this.x = x;
        this.y = y;
        ...
        if (client.getLevel() != null) {
            ...
            if (target != null && target.isSamePlace(client.getLevel())) {
                ...
                if (mount instanceof TransformationMountMob) {
                    ...
                    transformation.secondaryClickRunClient(client.getLevel(), x, y, target.playerMob);
                }
            }
        }
    ...
    public void processServer(NetworkPacket packet, Server server, ServerClient client) {
        if (client.slot == this.slot) {
            ...
            if (mount instanceof TransformationMountMob) {
                TransformationMountMob transformation = (TransformationMountMob) mount;
                if (transformation.canRunSecondaryClick(client.playerMob)) {
                    transformation.secondaryClickRunServer(client.getLevel(), x, y, client.playerMob);
                    server.network.sendToClientsAtEntireLevelExcept(new TransformationAbility2Packet(this.slot, this.x, this.y), client.getLevel(), client);
                }
            }
        } else {
